 ...
 * generates four tiny stub files that re-export from a compiled agentmesh
 * bundle.
 *
 ...
 * under <plugins-dir>/agentmesh-codex/ first. The compiled bundle loads on
...
async function findTsc() {
  ...
  try {
    ...
    try {
      ...
      if (npx.status !== 0) {
        throw new Error("agentmesh installer: typescript not found. Run `bun install` in the agentmesh checkout first.")
      }
      return null
    }
  }
...
async function compileEntry() {
  ...
  if (!tsc) throw new Error("agentmesh installer: could not locate tsc")
  const outDir = await mkdtemp(join(tmpdir(), "agentmesh-codex-"))
  ...
  if (result.status !== 0) {
    ...
    throw new Error("agentmesh installer: tsc failed to compile the agentmesh entry graph")
  }
  return outDir
}
...
const bundleDir = join(pluginsDir, "agentmesh-codex")
 ...
 */
import { buildDefinitions, handlers } from "./agentmesh-codex/codex.js"
...
console.log(`Note: the swarm sidecar still requires Node >= 23.6 on PATH (or AGENTMESH_NODE).`)
